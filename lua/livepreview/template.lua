...
local function ejs2html(template, initial_data)
	local container = [[
        <div id="livepreview-container" class="markdown-body"></div>
    ]]
	local script = [[
		<script>
			...
			const livepreview_renderTemplate = (tpl, data) => {
				...
				let html = safeTpl.replace(/<%-\s*([\s\S]*?)\s*%>/g, replaceValue(true));
				html = html.replace(/<%=\s*([\s\S]*?)\s*%>/g, replaceValue(false));
				return html;
			};
			...
			const livepreview_fetchData = async () => {
				try {
					...
					console.warn("live-preview.nvim: failed to load EJS data", err);
					return livepreview_initialData;
				}
			};
